...
if (ACTIVE_BEAM == 1) { // CANTILEVER BEAM WITH END MOMENT ONLY
    ...
    ELEM = concat([[Qstart,LN,t*8,w,0]] ,concat(ELEM_VIS,[[Qend,LN,t,w,0]]));  
...
// Roark 6th ed, Table 3, case 1a and 3a, page 100
    echo("CASE 2 Catilever beam, left fixed, right free, end load ");
    ...
    M2 = 0;  // 6, 4  , -7
        ...
        color("black") draw_points(RoarkPts,dia=0.03);   
    ...
    START_BEAM = [[Qstart,LEN/N_BEAMS,t*3,w,0]];
    ...
    DO_ANALYSIS(LDB=BEAM,EXT_LOADS=LOADS1,fscale=0.2,Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=FAILURE_STRESS_PLA_PSI, E=E_PLA_PSI, density=DENSITY_PLA_IMPERIAL,steps=6);
    
} else if (ACTIVE_BEAM == 3) { // Sinewave beam
    // TO DO:  REPLACE WITH ARC BEAM, AND ADD ROARK SOLUTIONS
    echo("CASE 3 SINE beam, left fixed, right pinned. ADD ARC BEAM TO MATCH ROARK ");
    ...
    echo("CASE 4: ROARK case 2a: L=3, t=0.15, w=.8, F=10 lb per inch ,Roark defection=1.2,min MS=-0.06");
    // Beam thickness
    t=.15;  // inch
    // Number of Beams
    NumberBeams=20;
    ...
    L=3;
    LN = L/NumberBeams;  // length of each beam segment
    Fy=10*(L/(NumberBeams+1)); // 1 LB PER INCH 
    TOTAL_LOAD = Fy*(NumberBeams+1);
    echo(LN=LN,Fy=Fy,TOTAL_LOAD=TOTAL_LOAD);
    
    // beam angle at fixed end
    ang_fixed = 30; // [-90:10:90]
    ...
    LOADS1 = concat([for (i=[1:NumberBeams]) [0,Fy*cos(ang_fixed),Fy*sin(ang_fixed)]],[[0,Fy*cos(ang_fixed),Fy*sin(ang_fixed)]]);
   
    MAKE_BEAM_UNDEFORMED(BEAM1,w);

    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*0.4, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=Failure_Stress, E=E_PSI,density=density,steps=8);

} else if (ACTIVE_BEAM == 5) { // CANTILEVER BEAM TEST CASE FROM OTHER SOURCE
    echo("TEST CASE 5: DATA FROM SOME OTHER PROGRAM ???");
    ...
    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*0.1, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=Failure_Stress, E=340000,density=density,steps=8);

} else if (ACTIVE_BEAM == 6) { // CANTILEVER BEAM TEST CASE F=0.278 lb
    echo("CASE 6, Polycarb Canteliver Beam with 0.278 lb at end");
    // TEST CASE: L=7 inch, t=0.062 inch, w=1.06 inch, F=0.278 lb
    ...
    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*1, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=Failure_Stress, E=340000,density=density,steps=8);
    
} else if (ACTIVE_BEAM == 7) { // CANTILEVER BEAM TEST CASE F=545G
    echo("CASE 7, Polycarb Canteliver Beam with 1.155 lb at end");
    // TEST CASE: L=7, t=0.062, w=1.06, F=1.155 lb
    E = 320000; // Polycarbonate
    density = 0.043;
    ...
    // NEED A LOT OF STEPS
    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*0.2, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=Failure_Stress, E=340000,density=density,steps=30);
    ...
    echo("CASE 9: Euler Column Load Limit is about 3 lb for t = 0.05,  L = 3");
    NumberBeams=20; 
    w=.8; // beam width
    ...
    t=0.05;
    LN = L/NumberBeams;
    Fx=-4.0; // -5.5 lbs,  HIGHER THAN EULER CALCULATION? IS IT PINNED ON BOTH ENDS??
    
    // SENSITIVE TO THE NUMBER OF BEAMS AND STEPS!!!
    
    BEAM_VIS = [[Qbeam,LN,t,w,0.0], for (i=[1:NumberBeams-1]) [Qbeam,LN,t,w,0]];
    BEAM1 = concat([[Qstart,LN,t*1.5,w,0.5]] ,concat(BEAM_VIS,[[Qend,LN,t*1.5,w,0]]));  
    LOADS1 = concat([for (i=[1:NumberBeams]) [0,0,0]],[[Fx,0,0]]);
   
    translate([0,-.2,0]) MAKE_BEAM_UNDEFORMED(BEAM1,w);

    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*0.2, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=Failure_Stress, E=340000,density=density,pinned=true,FixFy=false,steps=60);

} else if (ACTIVE_BEAM == 10) { // Frame with pinned supports
    echo("CASE 10: Roark 6th ed, Rigid Frame, Table 4, page 122, case 1f, where a = L1");
    ...
    DO_ANALYSIS(LDB=BEAM1, EXT_LOADS=LOADS1, fscale=force_scale*1, Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=FAILURE_STRESS_PLA_PSI, E=E,density=DENSITY_PLA_IMPERIAL,pinned=true,FixFy=true,steps=10);
...
//
    ...
    pts0 = [[0,0],[WIDTH,HGT],[0,HGT],[WIDTH,0]];
    pts1 = addPoints(pts0,0.5);
    color("blue") draw_points(pts1,dia=0.1);
    BEAM1= BEAM_FROM_NODES(nodes=pts1,TBEAMS=t,TENDS=t,w=w,THICKEN_ENDS=false,T_MID=true,TUP = 1.3,S=3);
    ...
    concat([for (i=[1:(NumberBeams/2)-1]) [0,0,0]],[[Fx,0,0]]), // first leg & top load
        [for (j=[1:(NumberBeams/2)-2]) [0,0,0]],
            // second leg
            [[-.019,0,0]])); // << last load is back solved to hold
       
    //echo(LOADS1=LOADS1," n=", len(LOADS1));

    MAKE_BEAM_UNDEFORMED(BEAM1,w);
    
    DO_ANALYSIS(LDB=BEAM1,EXT_LOADS=LOADS1,fscale=0.9,Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=FAILURE_STRESS_PLA_PSI, E=E_PLA_PSI, density=DENSITY_PLA_IMPERIAL,pinned=true,FixFy=true,steps=20);
    ...
    BEAM1= BEAM_FROM_NODES(nodes=pts,TBEAMS=t,TENDS=t,w=w,THICKEN_ENDS=true,T_MID=false,TUP = 1.3,S=3);

    LOADS1 = concat([for (i=[1:NumberBeams]) [0,0,0]],[[Fx,0,0]]);
   
    translate([0,-10,0]) MAKE_BEAM_UNDEFORMED(BEAM1,w); 

    DO_ANALYSIS(LDB=BEAM1,EXT_LOADS=LOADS1,fscale=0.9,Display_steps=true, echoLDB=false, displayLoads=true, Failure_Stress=FAILURE_STRESS_PLA_METRIC, E=E_PLA_NSMM, density=DENSITY_PLA_METRIC,pinned=true,FixFy=true,steps=20);
