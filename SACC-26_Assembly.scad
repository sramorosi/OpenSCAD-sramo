...
display_assy = true;
// Draw the End effecot, belts, springs 
display_all = false;
// Section cut at X = 0?
clip_yz = false;
// Section cut at Z = 0?
    ...
    rotate([0,0,A_angle]) {
        ...
        color("plum",1) 
            translate ([0,0,-widthAB/2])
                final_AB_arm (spr_dist_AB=spr_dist_AB);
        // Draw the BC link
        color("blue",1) 
...
module final_AB_arm (lenAB=9,r_pulley=1,spr_dist_AB=3) {
    ...
    difference () {
        union () {
            ...
            //translate([0,0,widthAB])
                //cylinder(h=a_svo_boss,d=widthAB,center=false);
        }
...
module final_BC_arm (lenBC=9,r_pulley=1) {
    $fa=$preview ? 6 : 1; // minimum angle fragment
    $fs=0.01; // minimum size of fragment (default is 2)
