...
rotate([0,0,45]) translate([L,0,0]) 
TRIGGER(Z_TRIG=30,Z_LAUNCH=Z_BEAM,W_TRIG=18);
...
module SHOOTER_SUB1() {
    ...
    BEAM1 = beamFromNodes(pts,t,Z_BEAM,THICKEN_ENDS=true,TUP=1.05); // make beam model from points
    
    //draw_beam_undeformed(BEAM1);
    StartingNodes = getNodesFromBeams(BEAM1,0,0);  
    

    FLEX_BEAM_FILLETED(BEAM1,BEAM_T=t,BEAM_W=Z_BEAM,BEAM_ANG=2,R=Rfillet, NODES=StartingNodes);

    translate([0,-BEAM_SPACE,0]) rotate([180,0,0]) 
        FLEX_BEAM_FILLETED(BEAM1,BEAM_T=t,BEAM_W=Z_BEAM,BEAM_ANG=0,R=Rfillet, NODES=StartingNodes);
...
module SHOOTER_SUB2() {
    difference() {
        ...
        rotate([0,90,0]) translate([3,0,0]) 
            cylinder(h=3*V_LEN,d=5.8,center=true,$fn=100);
        ...
        translate([L+1.5*END_T,0,0]) SCREW_HOLES(DIA=2.5);
...
module TRIGGER(Z_TRIG=10,Z_LAUNCH=5,W_TRIG=20) {
    ...
    color("blue") {
        difference() {
            ...
            cube([10,W_SQR*1.1,Z_LAUNCH],center=true); // notch
            
            translate([-5,0,0]) 
                rotate([0,-30,0]) 
                    cube([10,W_TRIG*2,Z_LAUNCH],center=true); // chamfer
        ...
        difference() {
            ...
            translate([25,0,0]) SCREW_HOLES(DIA=3.3); 
        }
        translate([2,-W_TRIG/2,-13.5]) 
            rotate([0,-45,0]) 
                cube([6,W_TRIG,2]); // fillet on flex beam, trigger side
        
        translate([30,-W_TRIG/2,-40-Z_LAUNCH/2]) 
            cube([8,W_TRIG,40]); // palm upright
        
        translate([28,-W_TRIG/2,-9.5]) 
            rotate([0,45,0]) 
                cube([6,W_TRIG,2]); // fillet on flex beam, trigger side
    }
}
